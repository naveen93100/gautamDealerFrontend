...
//     const pages = state?.proposalType === 'Both' ? [
//         "galo1.jpeg",
//         "galo2.jpeg",
//         "galo3.jpeg",
//         "galo4.jpeg",
...
//                 {/* <GaloPdfComp bg={pages[4]}></GaloPdfComp> */}
...
const GaloSalesProposalView = () => {
    ...
    // panel entries and has no subsidy field.
    const panelRows = state?.selectedPanels || [];
    const panelSubtotal = panelRows.reduce(
        (sum, p) => sum + Number(p?.totalPrice || 0),
        0,
    );
    const panelGstTotal = panelRows.reduce(
        (sum, p) => sum + Number(p?.gstAmount || 0),
        0,
    );
    const panelGrandTotal = panelSubtotal + panelGstTotal;
    ...
    return state?.proposalType === "Both" ? (
        <div className="flex min-h-screen flex-col gap-6  bg-gray-100">
            {/* --- Back Button --- */}
            <button
                onClick={() => navigate(-1)}
            ...
            <GaloPdfComp bg={pages[4]}>
                ...
                <div className="absolute top-[94mm] right-[-10mm] w-100 text-center">
                    <div>
                        <h1 className="text-md font-semibold italic text-black">
                            Galo Solar{" "}
                            {state?.selectedPanels?.[0]?.wattId?.watt}
                            Wp
                        </h1>
                    </div>
                </div>
        ...
        <div className="flex min-h-screen flex-col gap-6  bg-gray-100">
            ...
            <GaloPdfComp bg={pages[2]}>
                ...
                {/* Efficiency/warranty ribbons removed — those figures were
                    tied to the inverter'd Both-mode plant spec, not a
                    standalone panel sale */}
            </GaloPdfComp>
            ...
            <GaloPdfComp bg={pages[3]}>
                <div className="absolute top-[60mm] left-[10mm] w-[190mm]">
                    ...
                    <table className="w-full border-2 border-black border-collapse text-center mb-6">
                        <thead>
                            <tr className="bg-black text-white">
                                <th className="border border-black py-2 w-16">
                                    S.No
                                </th>
                                <th className="border border-black py-2">
                                    Panel Watt
                    ...
                    <table className="w-full border-2 border-black border-collapse text-center">
                        <thead>
                            <tr className="bg-black text-white">
                                <th className="border border-black py-2 w-14">
                                    S.No
                                </th>
                                <th className="border border-black py-2">
                                    Item Description
                                </th>
                                <th className="border border-black py-2 w-24">
                                    ₹ Rate/Watt
                                </th>
                                <th className="border border-black py-2 w-20">
                                    Quantity
                                ...
                                <th className="border border-black py-2 w-28">
                                    ₹ Amount + GST
                                </th>
                            </tr>
                        </thead>
                        <tbody className="bg-[#fdf0c7]">
                            {panelRows.map((panel, idx) => {
                                const amount = Number(panel?.totalPrice || 0);
                                const gstAmt = Number(panel?.gstAmount || 0);
                                return (
                                    <tr key={idx}>
                                        <td className="border border-black py-3">
                                            {idx + 1}
                ...
                <div className="absolute bottom-[30mm] left-[10mm] w-[190mm]">
                    ...
                    <ul className="list-disc pl-5 space-y-1 text-black text-sm leading-relaxed">
                        <li>Delivery Readiness after Full Payment.</li>
                        <li>
                            Payment: RTGS/NEFT ({bank?.bank} A/C:{" "}
                            {bank?.account}
                            <br />
                            IFSC: {bank?.ifsc}).
                        </li>
                        <li>
                            Required: Electricity Bill, Aadhaar, Bank
