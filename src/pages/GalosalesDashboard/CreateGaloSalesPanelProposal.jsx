...
//     // adding the toggle state  for the create the proposal
//     const [proposalType, setProposalType] = useState("Both");
...
const CreateGaloSalesPanelProposal = ({
    ...
    // Single source of truth for a fresh/blank panel row —
    // used on initial state, on toggling proposal type, and on close.
    const blankPanel = () => ({
        panelId: "",
        technologyId: "",
        constructiveId: "",
    ...
    const [selectPanel, setSelectPanel] = useState([blankPanel()]);
...
`);
    const handleClose = () => {
        setCreatePanelData({ gst: 8.9 });
        setSelectPanel([blankPanel()]);
        setActiveIndex(0);
        setProposalType("Both");
        onClose(false);
    };

    // Switching Both <-> Panel clears entered panel data instead of
    // carrying stale/hidden field values across into the new mode.
    const handleProposalTypeChange = (type) => {
        if (type === proposalType) return;
    ...
    return (
        <div className="fixed inset-0 bg-[#14141A]/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[#FAFAF7] rounded-2xl shadow-2xl border border-[#E7E3D8] max-w-3xl w-full max-h-[92vh] flex flex-col overflow-hidden">
                {/* ===== Header ===== */}
                <div className="bg-[#14141A] px-6 py-5 flex items-center justify-between border-b-2 border-[#F5A623] shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="w-11 h-11 bg-[#F5A623] rounded-xl flex items-center justify-center shrink-0">
                            <Sun className="w-5 h-5 text-[#14141A]" />
                        </div>
                        <div>
                            ...
                            <p className="text-[#B8B6AC] text-sm mt-0.5">
                                {!data || data?.panelData?.length === 0
                                    ? "Build"
                                    : "Update"}{" "}
                                a detailed panel proposal for your customer
                    ...
                    <button
                        onClick={handleClose}
                        className="p-2 rounded-full text-[#B8B6AC] hover:bg-white/10 hover:text-white transition shrink-0"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="px-6 py-4 border-b border-[#E7E3D8] bg-white shrink-0">
                    <div className="inline-flex bg-[#F1EFE7] rounded-full p-1">
                        <button
                            type="button"
                ...
                <form className="flex flex-col flex-1 min-h-0">
                    <div className="px-6 py-6 overflow-y-auto flex-1 space-y-8">
                        ...
                        {/* GST Section */}
                        <section>
                            <div className="flex items-center gap-2 mb-4 pb-2 border-b border-[#E7E3D8]">
                                <IndianRupee className="w-4 h-4 text-[#F5A623]" />
                                <h3 className="text-[15px] font-semibold text-[#14141A]">
                                    Price and GST
                                </h3>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm text-[#6B6A63] mb-1.5">
                                        GST (%)
                                    </label>
                                    <input
                                        type="number"
                                        ...
                                        value={createPanelData?.gst}
                                        onChange={handleChange}
                                        className="w-full px-4 py-2.5 border border-[#E7E3D8] rounded-lg bg-white text-[#14141A] focus:ring-2 focus:ring-[#F5A623]/30 focus:border-[#F5A623] outline-none transition"
                                        placeholder="GST %"
                                        min={1}
                                        onWheel={(e) => e.target.blur()}
                                        onKeyDown={(e) => {
                        ...
                        <section>
                            ...
                            <div className="rounded-lg border border-[#E7E3D8] overflow-hidden">
                                <JoditEditor
                                    key={company}
                                    config={joditConfig}
                                    value={Body}
                    ...
                    {/* ===== Sticky Footer ===== */}
                    <div className="shrink-0 border-t border-[#E7E3D8] bg-white px-6 py-4 flex justify-end gap-3">
                        <button
                            type="button"
                            onClick={handleClose}
                            className="px-5 py-2.5 rounded-lg border border-[#E7E3D8] text-[#14141A] text-sm font-medium hover:bg-[#F1EFE7] transition"
                        >
                            Cancel
                        </button>
