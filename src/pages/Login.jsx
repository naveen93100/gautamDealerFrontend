...
//             <span onClick={() => setAdmin(!admin)} className="text-red-700  cursor-pointer hover:underline">{admin ? "Dealer Login" : "Admin Login"}</span> 
//             <span className="text-red-700 cursor-pointer  hover:underline gap-5">Sales Login</span>
//             </div>
//           </div>
...
const Login = () => {
  ...
  // Switch login type and reset form fields
  ...
  const titles = {
    dealer: { heading: "Dealer Login", sub: "Access your solar partnership dashboard" },
      ...
      <div className="bg-white p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] w-full shadow-gray-400 max-w-md border border-gray-300">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          {titles[loginType].sub && (
            <p className="text-gray-500 text-sm mt-1">{titles[loginType].sub}</p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {loginType === "sales" && (
            <div className="space-y-1.5">
              ...
              {errors.userId && (
                <p className="text-[11px] font-semibold text-red-600 ml-1 uppercase">{errors.userId.message}</p>
              )}
            </div>
          )}
          ...
          <div className="text-center pt-4 space-y-2">
            ...
            <div className="flex items-center justify-center gap-5">
              {loginType !== "dealer" && (
                <span onClick={() => switchLoginType("dealer")} className="text-red-700 cursor-pointer hover:underline text-sm">
                  Dealer Login
                </span>
              )}
              {loginType !== "admin" && (
                <span onClick={() => switchLoginType("admin")} className="text-red-700 cursor-pointer hover:underline text-sm">
                  Admin Login
                </span>
              )}
              {loginType !== "sales" && (
